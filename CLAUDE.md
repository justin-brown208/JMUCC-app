...
**Step 6 (Calendar) — built, blocked on OC config.** Client-side Google Calendar: `app/src/calendarConfig.ts` (API key + 9 public calendar IDs + full-week image URL — all `PASTE_` placeholders), `app/src/schedule.ts` (per-profile calendar resolution + Calendar API fetch/merge + localStorage cache), `ScheduleWidget` on Home (featured event large → later smaller → View Full Week), `app/src/screens/FullWeek.tsx` (`react-zoom-pan-pinch` viewer on the OC image). **Blocked on:** OC creating the 9 public calendars + a Calendar API key, then filling `calendarConfig.ts` (widget shows empty state until then). Verify fetch mechanism against any public calendar once a key exists.
