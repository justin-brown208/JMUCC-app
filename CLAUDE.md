...
- [2026-09-07] **Document search: prompt caching + Haiku primary** (cuts ~85% of per-query cost). Measured the actual spend first: the rulebook is 13 pages / ~8.1k tokens and was re-sent in full on every query → ~$0.021/question, ~$50 across a 500-person week. Two changes, no retrieval: (a) the document text moves into its own content part marked `cache_control: {type:"ephemeral"}` with the **question appended after** it, so the cached prefix (system + docs) is byte-identical every time — reads bill at **0.1x**, writes 1.25x, so it pays for itself on the 2nd question inside the 5-minute window (`CACHE_CONTROL` is the one-line switch to a 1h/2x window). (b) primary model → **`anthropic/claude-haiku-4.5`** ($1/$5 vs Sonnet's $2/$10), Sonnet 5 demoted to fallback; the task is extraction and the verbatim substring guard already blocks invention, so the cheaper model risks worse *ranking*, never a bad quote. Together ≈ **$0.003/question**. Explicitly **rejected chunk-retrieval**: it conflicts with caching (a varying chunk set destroys the prefix), and at 13 pages it would add a silent wrong-section failure mode the verbatim guard cannot catch. **Consequence:** the fallback is now Anthropic→Anthropic, so an Anthropic-wide outage takes search down — append `openai/gpt-5` to `MODELS` to restore cross-provider cover.
...
- [2026-08-24] **Academic queue has no claim step** — it's worked by the single VP Academics, so tickets go `open → resolved` directly (no `claimed` state, no Claim/Drop UI; `QueuePanel` `noClaim`). Dedicated rule branch (`queue == "academic"`, `open → resolved`); resolve push broadened to fire on any `→ resolved`. Tech/runner keep `claim → resolve`. Deployed + E2E-verified (Coach submits, VP resolves from open).
- [2026-08-24] **Nav → bottom tab bar** (role-aware icon-only: Home/Rules/Messages + Requests if submit-or-work + Admin if `isAdmin`); Home is now a content-only dashboard (greeting tap → Log out). A worker's queue is the **main content of the Requests tab** (inline `QueuePanel`), not a separate My Queue screen (removed). **Buttons/chips are now outlined, no fill** (transparent + outline encodes state: gold = primary/selected, muted = secondary/unselected); fills reserved for passive surfaces (cards, inputs). Calendar entries are gray-outlined cards. `DESIGN.md` updated (component A/C + grammar).
